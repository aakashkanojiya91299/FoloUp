-- Applications Feature Database Schema
...
-- Create RLS policies for interviewees with more permissive access
...
CREATE POLICY "Users can view resume analyses from their organization" ON resume_analyses
    FOR SELECT USING (
        resume_id IN (
            SELECT id FROM resumes WHERE interviewee_id IN (
                SELECT id FROM interviewees WHERE organization_id IN (
                    SELECT organization_id FROM "user" WHERE id = auth.uid()::text
                )
            )
...
-- Create RLS policies for ATS job requirements
