-- Create enum type for link status
CREATE TYPE link_status AS ENUM ('active', 'expired', 'completed');

-- Create table for candidate interview links
...
CREATE INDEX idx_candidate_interview_links_candidate ON candidate_interview_links(candidate_id);
CREATE INDEX idx_candidate_interview_links_interview ON candidate_interview_links(interview_id);
CREATE INDEX idx_candidate_interview_links_organization ON candidate_interview_links(organization_id);
CREATE INDEX idx_candidate_interview_links_unique_link ON candidate_interview_links(unique_link_id);
CREATE INDEX idx_candidate_interview_links_status ON candidate_interview_links(status);

-- Enable Row Level Security
ALTER TABLE candidate_interview_links ENABLE ROW LEVEL SECURITY;

-- RLS Policies for candidate_interview_links
