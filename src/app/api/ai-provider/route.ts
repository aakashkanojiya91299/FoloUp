...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!provider || !organizationId || !userId) {
      return NextResponse.json({ error: "Provider, organization ID, and user ID are required" }, { status: 400 });
    }
    ...
    const preference = await setAIProviderPreference(organizationId, userId, provider as AIProvider);
