  ...
  try {
    ...
    if (!jobTitle || !jobDescription || !organizationId || !userId) {
      return NextResponse.json(
        {
          error:
            "Job title, description, organization ID, and user ID are required",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      if (userPreference) {
        ...
        console.log(
          `Generate Questions: Using user preference: ${preferredProvider}`,
        );
      } else {
        ...
        if (orgPreference) {
          ...
          console.log(
            `Generate Questions: Using organization preference: ${preferredProvider}`,
          );
        } else {
          console.log(
            `Generate Questions: No preference found, using default: ${preferredProvider}`,
          );
        }
      }
    } catch (prefError) {
    ...
    console.log(
      `Generate Questions: AI service provider set to: ${aiService.getCurrentProvider()}`,
    );
    ...
    if (error.message?.includes("quota") || error.status === 429) {
      return NextResponse.json(
        {
          error:
            "AI service quota exceeded. Please try again later or switch to a different AI provider.",
        },
        { status: 429 },
      );
    }
    ...
    if (error.status === 401) {
      return NextResponse.json(
        {
          error:
            "AI service authentication failed. Please check your API keys.",
        },
        { status: 401 },
      );
    }
